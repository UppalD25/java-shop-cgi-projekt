...
public class CartProductDAO implements ICartProduct {
    ...
    public int getTotalItemsInCart(int shoppingCartId) {
        String sql = "SELECT Nz(SUM(Quantity), 0) as total FROM CartProduct WHERE ShoppingCart_ShoppingCart_ID = ?";
    ...
    public double getCartSubtotal(int shoppingCartId) {
        String sql = "SELECT Nz(SUM(Price * Quantity), 0) as subtotal FROM CartProduct WHERE ShoppingCart_ShoppingCart_ID = ?";
